...
public sealed class SettingsDialog : Window
{
	...
	private static Panel BuildReleaseNotes()
	{
		...
		stackPanel.Children.Add(new TextBlock
		{
			Text = "MISE 1.1.21 – 2026/09/07\n・Undo復元を保存データ移行から分離\n・アクティブ選択の復元とフォント追加履歴を修正\n・4ページPDF／AI読込と状態復元テストを追加\n\nMISE 1.1.20 – 2026/09/02\n・ロック中の要素を削除、移動、整列、複製、装飾変更、レイヤー変更から一貫して保護\n・ロック要素が混在する複数選択では編集可能な要素だけを操作\n・面付け元ページを指定した出力DPIで描画し、細い線・文字・QRの劣化を防止\n・タイト文字の非等方拡大を廃止し、縦横比を保ったままフレーム中央へ配置\n・画像DPIを幅と高さの低い方で判定し、誤った品質表示を修正\n・埋め込みフォント検証、保存形式2への移行、PDF／AI原本切れ警告を追加\n・PDF／AI編集読込の制約を明示し、安全な既定値へ変更\n・複数選択時のアクティブ対象を固定\n\nMISE 1.1.19 – 2026/09/02\n・幅、高さ、縦横比固定を共通の変形計算へ統合\n・縦横比固定中は幅の変更に高さ、高さの変更に幅を即時連動\n・ハンドル操作中も幅、高さ、位置、文字サイズをプロパティへリアルタイム反映\n・タイトフレーム文字はフォントサイズ、字間、行間、縁取り、立体量を同じ倍率で拡大縮小\n・文字の最小3ptと最大300ptでフレーム寸法も同時に制限し、表示と数値のずれを防止\n・QRコードは常に1:1かつ最小5mm、画像と図形は設定された比率固定状態に従う\n・数値スピン操作時の無変更履歴を除き、1操作1回のUndoへ整理\n\nMISE 1.1.18 – 2026/09/02\n・選択オブジェクトの拡大縮小ハンドルを画面ズームへ連動\n・25～400％のどの倍率でも、ハンドルを画面上約9pxの操作サイズで維持\n・高倍率ズーム時にハンドルが文字や画像を覆う問題を修正\n・回転ハンドルの大きさとオブジェクトからの距離もズーム補正\n・選択枠の線幅をズーム補正し、倍率による太さの変化を抑止",
			TextWrapping = TextWrapping.Wrap,
			LineHeight = 22.0,
			FontSize = 13.0,
		...
		stackPanel.Children.Add(new TextBlock
		{
			Text = "MISE 1.1.17 – 2026/09/02\n・Ctrl＋Z／Ctrl＋Yを入力欄ごとの履歴ではなく、MISE全体の操作履歴へ統一\n・文字入力欄や数値入力欄にフォーカスがある状態でも、全体のUndo／Redoを実行\n・Ctrl＋Shift＋Zによるやり直しにも対応\n・Undo／Redo時に操作対象ページと選択オブジェクトを復元\n・文字オブジェクトで縦横比維持設定が無視される問題を修正\n・縮小時の最小幅・最小高さを同一倍率で制限し、一定サイズ以下で横幅だけ縮む問題を修正\n・文字の拡大縮小と数値入力の最小フォントサイズを3ptへ統一\n・縮小後も文字フレーム、フォントサイズ、字間、行間、縁取り、立体効果の倍率を統一",
			TextWrapping = TextWrapping.Wrap,
			LineHeight = 22.0,
			FontSize = 13.0,
			FontWeight = FontWeights.SemiBold,
		...
		stackPanel.Children.Add(new TextBlock
		{
			Text = "MISE 1.1.16 – 2026/08/29\n・PDF／PDF互換Illustrator AIの読み込み画面へ「編集優先」「見た目優先」「詳細設定」を追加\n・詳細設定で文字と画像を別々に編集可能オブジェクトへ変換するか選択可能\n・未導入フォントを含む文字を固定背景に残し、元の見た目を維持する選択肢を追加\n・画像だけを編集可能にし、文字は元デザインのまま保持する読み込み構成に対応\n・固定背景の品質を標準（長辺2600px）／高精細（長辺5200px）から選択可能",
			TextWrapping = TextWrapping.Wrap,
			LineHeight = 22.0,
			FontSize = 13.0,
			FontWeight = FontWeights.SemiBold,
		...
		stackPanel.Children.Add(new TextBlock
		{
			Text = "MISE 1.1.15 – 2026/08/28\n・新規文字の初期フォントサイズを34ptへ変更\n・文字の拡大縮小ハンドルを図形と同じカーソル追従方式へ変更\n・掴んだ点がマウスから離れる原因だった文字の常時縦横比固定を廃止\n・通常ドラッグは自由拡縮、Shift＋ドラッグ時のみ縦横比を固定\n・自由拡縮中もフォントサイズ、字間、行間、縁取り、立体効果を拡縮量へ連動\n・文字の描画外周をフレーム内へ再フィットし、選択枠と文字のずれを抑止\n・ドラッグ中のマウスキャプチャを明示し、ハンドル操作の取りこぼしを防止",
			TextWrapping = TextWrapping.Wrap,
			LineHeight = 22.0,
			FontSize = 13.0,
			FontWeight = FontWeights.SemiBold,
		...
		stackPanel.Children.Add(new TextBlock
		{
			Text = "MISE 1.1.14 – 2026/08/28\n・新しく挿入した文字の選択フレームを、文字・縁取り・立体効果の外接範囲へ自動調整\n・文字フレームのドラッグ拡大縮小に、フォントサイズ、字間、行間、縁取り、立体効果を比例連動\n・既存プロジェクトの文字フレームは従来サイズを維持し、レイアウトの意図しない変化を防止\n・PDF／PDF互換Illustrator AIを開く際に「編集優先」と「見た目優先」を選択可能\n・編集優先では検出した文字と埋め込み画像を個別オブジェクト化し、画像の移動・拡縮・削除に対応\n・未導入フォントを近いシステムフォントへ置換し、置換発生を読込完了時に通知\n・見た目優先では各ページを長辺最大5200pxで高精細画像化し、元デザインの外観を維持",
			TextWrapping = TextWrapping.Wrap,
			LineHeight = 22.0,
			FontSize = 13.0,
			FontWeight = FontWeights.SemiBold,
		...
		stackPanel.Children.Add(new TextBlock
		{
			Text = "MISE 1.1.13 – 2026/08/28\n・文字内容欄の入力・削除・貼り付けをキャンバスへリアルタイム反映\n・入力中のカーソル位置を維持し、連続編集を1回のUndoとして記録\n・PDFとPDF互換Illustrator AIを編集用プロジェクトとして直接開く機能を追加\n・元デザインを高精細な固定背景として保持し、検出した文字を編集可能な要素へ変換\n・複数ページ、文字色、文字サイズ、フォント名、配置、回転の取り込みに対応\n・PDF互換データを含まないAIにはIllustratorでの再保存手順を表示",
			TextWrapping = TextWrapping.Wrap,
			LineHeight = 22.0,
			FontSize = 13.0,
			FontWeight = FontWeights.SemiBold,
